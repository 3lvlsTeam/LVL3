...
def signup():
    if request.method=="POST":
        ...
        if email:
            flash("This Email is already used, try another one.")
            allgood= False
...
def signupF2():   
    make_tmp_usr()
    if not loged_lvl_1():
        flash("Finish sign up in level 1 first !")
        return redirect(url_for('login'))
    if usr.img_password !=  None:
        if loged_lvl_2():
            pass
        else:
            flash("Finish loging in level 2 first !")
            return redirect(url_for('login'))
    if request.method == "POST":
        ...
        else:flash("Choose five images at least!!")
...
def signupF3():
    make_tmp_usr()
    if not loged_lvl_2():
        flash("finish sign up in level 2 first!")
        return redirect(url_for('login'))
    if usr.hashed_img != None:
        if loged_lvl_3():
            pass
        else:
            flash("finish loging in level 3 first !")
            return redirect(url_for('login'))
    flash("NOTE: If the image did not change use CTRL+F5 ")
    if request.method == "POST":
    ...
    try:
        ...
        flash("No image was uploaded yet!")
...
def uploader():
    ...
    if len(img_list)<5:
        flash("Must be at least five photos ")
    if request.method=="POST":
    ...
    if request.method=="POST":
        ...
        if file and functions.allowed_file(file.filename):
            ...
            if img.width != 1000 or img.height != 1000:
                flash("Image size was adjusted to fite.")
                img.thumbnail((1000,1000))
                img.save(directory+"/convesbackground.png")
                flash("if the image did not change use CTRL+F5 ")
        else:
            flash('Allowed image types are - png, jpg, jpeg, gif')      
...
def loginF2():
    ...
    if loged_lvl_1():       
        if usr.img_password == None:
            flash("Please Fnish Signing Up First!")
            return redirect(url_for("signupF2"))
        ...
        return render_template("loginF2.html", imgs_list =img_list, directory=directory ,usr=usr)
            ...
            flash("Finish loging in level 1 first !")
            return redirect(url_for("login") )
...
def loginF3():
    ...
    if loged_lvl_2() :       
        if usr.hashed_img == None:
            flash("Please Finish Signing Up First!")
            return redirect(url_for("signupF3"))
        if request.method=="POST":
            session['hashed_img']=str(functions.hash_this_img(request.form["img_src"]))
            if loged_lvl_3():
...
def home():
    if loged_lvl_1() and loged_lvl_2() and loged_lvl_3():
        ...
        flash("You are not loged in yet!")
        return redirect(url_for("login"))
#------------------------------------------------------------------------------------------------------------------------------
